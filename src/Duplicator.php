...
class Duplicator extends Container
{
	...
	{
		return $this[$name] = new self::$containerClass;
	}
	 ...
	 * @param string $name
	 */
	public static function register($name = 'addDuplicator', $container = '') {
		...
		Container::extensionMethod($name, function (Container $form, $name, $factory, $copyNumber = 1, $maxCopies = null) {
                    ...
                    $duplicator->setCurrentGroup($form->getCurrentGroup());

                    return $form[$name] = $duplicator;
                });
			...
			$_this->onClick[] = function (SubmitButton $button) use ($callback) {
				$replicator = $button->lookup(Duplicator::class);
